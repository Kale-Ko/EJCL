package io.github.kale_ko.ejcl.legacy;

import io.github.kale_ko.bjsl.elements.ParsedElement;
import io.github.kale_ko.bjsl.elements.ParsedPrimitive;
import io.github.kale_ko.bjsl.processor.ObjectProcessor;
import io.github.kale_ko.bjsl.processor.TypeProcessor;
import java.io.File;
import java.net.*;
...
/**
 * Container for type processors that are compatible with the old database format. For use in migration.
 *
 * @version 5.0.0
 * @since 5.0.0
 * @deprecated Only for use in migration.
 */
@Deprecated(forRemoval=true)
public class LegacyDefaultTypeProcessors {
    private LegacyDefaultTypeProcessors() {
    }

    /**
     * Registers the legacy default type processors for a given builder. This also unregisters all other default type processors.
     *
    ...
    public static void register(@NotNull ObjectProcessor.Builder builder) {
        if (builder.hasTypeProcessor(UUID.class)) {
            builder.removeTypeProcessor(UUID.class);
        }
        ...
        if (builder.hasTypeProcessor(URI.class)) {
            builder.removeTypeProcessor(URI.class);
        }
        ...
        if (builder.hasTypeProcessor(URL.class)) {
            builder.removeTypeProcessor(URL.class);
        }
        ...
        if (builder.hasTypeProcessor(Path.class)) {
            builder.removeTypeProcessor(Path.class);
        }
        ...
        if (builder.hasTypeProcessor(File.class)) {
            builder.removeTypeProcessor(File.class);
        }
        ...
        if (builder.hasTypeProcessor(InetAddress.class)) {
            builder.removeTypeProcessor(InetAddress.class);
        }
        ...
        if (builder.hasTypeProcessor(InetSocketAddress.class)) {
            builder.removeTypeProcessor(InetSocketAddress.class);
        }
        ...
        if (builder.hasTypeProcessor(Calendar.class)) {
            builder.removeTypeProcessor(Calendar.class);
        }
        ...
        if (builder.hasTypeProcessor(Date.class)) {
            builder.removeTypeProcessor(Date.class);
        }
        ...
        if (builder.hasTypeProcessor(Instant.class)) {
            builder.removeTypeProcessor(Instant.class);
        }
